...
pub const Scene = struct {
    ...
    pub fn encode(self: *Self) ![]c.vec4 {
        var buf = std.ArrayList(c.vec4).init(self.alloc);
        var mat_indexes = std.ArrayList(u32).init(self.alloc);
        defer mat_indexes.deinit();

        try buf.append(.{ .x = 0, .y = 0, .z = 0, .w = 0 });
        for (self.materials.items) |m| {
            try mat_indexes.append(@intCast(u32, buf.items.len));
            try m.encode(&buf);
        }
        buf.items[0].x = @bitCast(f32, @intCast(u32, buf.items.len));
        for (self.shapes.items) |s| {
            try s.encode(self.materials.items, mat_indexes.items, &buf);
        }
        buf.items[0].y = @bitCast(f32, @intCast(u32, buf.items.len));
        ...
        for (buf.items) |b| {
            ...
            for ([_]f32{ b.x, b.y, b.z, b.w }) |v| {
                std.debug.print("{}\t{}\t", .{ v, @bitCast(u32, v) });
            }
            std.debug.print("\n", .{});
        }
